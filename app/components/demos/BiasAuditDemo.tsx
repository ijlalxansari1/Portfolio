  ...
  const startAudit = () => {
    ...
    setTimeout(() => {
      setResults({
        ...
        protectedGroups: ["Gender", "Ethnicity", "Age"],
        riskLevel: "Medium",
        violations: 2
      });
      setIsAuditing(false);
  ...
  return (
    <div className="w-full bg-[#0d0d0d] border border-[var(--border)] rounded-2xl p-6 overflow-hidden min-h-[440px]">
      <div className="flex items-center gap-3 mb-8 pb-4 border-b border-[var(--border)]">
        ...
        <div>
          <h3 className="text-[14px] font-black text-white uppercase tracking-wider">Bias Audit System</h3>
          <p className="text-[10px] text-[var(--text-secondary)] opacity-50 uppercase tracking-widest">Model Governance & Fairness</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-4">
            <div className="p-5 bg-white/5 border border-[var(--border)] rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[12px] font-black text-white">Sensitive Attributes</p>
                  <p className="text-[10px] text-[var(--text-secondary)] opacity-50 uppercase tracking-widest">3 detected in metadata</p>
                </div>
              </div>
            ...
            <div className="p-5 bg-white/5 border border-[var(--border)] rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[12px] font-black text-white">Target Variable</p>
                  <p className="text-[10px] text-[var(--text-secondary)] opacity-50 uppercase tracking-widest">loan_approved_binary</p>
                </div>
              </div>
              <span className="px-2 py-0.5 bg-blue-500/20 text-blue-400 text-[9px] font-black uppercase rounded">Auto-Mapped</span>
            </div>
          </div>
          ...
          <div className="bg-orange-500/5 border border-orange-500/20 rounded-xl p-4 flex gap-3">
            ...
            <p className="text-[11px] text-orange-200/70 leading-relaxed">
              This system will calculate the Disparate Impact Ratio across all sensitive groups using the 4/5ths Rule of thumb.
            </p>
          </div>
          ...
          >
            ...
            {isAuditing ? "Auditing Metadata..." : "Run Bias Audit"}
          </button>
        </div>
      ) : (
        <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="space-y-6">
          <div className="text-center p-6 bg-orange-500/5 border border-orange-500/20 rounded-[28px]">
            <p className="text-[10px] font-black text-orange-500 uppercase tracking-[3px] mb-2">Disparate Impact Ratio</p>
            ...
            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${results.riskLevel === 'Medium' ? 'bg-orange-500/20 text-orange-400' : 'bg-red-500/20 text-red-400'}`}>
              <AlertTriangle size={12} /> {results.riskLevel} Risk Detected
            </div>
          </div>
